...
namespace Examine.Test.Examine.Lucene.Extensions
{
    ...
    public class SpatialSearch : ExamineBaseTest
    {
        ...
        {
            var gotIds = new string[docs.TotalHits];
            for (var i = 0; i < gotIds.Length; i++)
            {
        ...
        {
            var lat = double.Parse(e.ValueSet.Values["lat"].First().ToString());
            var lng = double.Parse(e.ValueSet.Values["lng"].First().ToString());
